...
import com.hengyuan.hicash.domain.query.mktApp.HinsTempCreditQuery;
...
* 类说明 :嗨女神创建正式授信订单
 */
public class HinsCreateCreditService implements RespService {

	private static Logger logger = Logger
			.getLogger(HinsCreateCreditService.class);

	private HinsTempCreditQuery tempCreditQuery = new HinsTempCreditQuery();
	...
	public synchronized ParmResponse responseValue(ParmRequest parmRequest) {
		...
		/* 创建申请件 */
		HinsCreateCreditResp resp = new HinsCreateCreditResp();
		String resultCode = "";
		String applicationNo = "";
		Long start = System.currentTimeMillis();
		HinsCreateCreditReq req = (HinsCreateCreditReq) parmRequest;
		 logger.info("临时单号:"+req.getTemp_no()+"开始创建授信正式订单");
		 HinsTempCredit tempCredit=new HinsTempCredit();
		...
		try {
			...
			if (Consts.FINAL_NUMBER_N.equals(tempCredit.getCreateapp_flag())) {
				 logger.info("临时单号:"+req.getTemp_no()+"未生成正式授信单,开始生成授信单");
				...
				logger.info("临时单号:"+req.getTemp_no()+"已生成正式授信单,不能再生成正式授信单");
			}
	...
	public String initCredit(HinsCreateCreditReq req,HinsTempCredit tempCredit,CustCustomer customer)
			throws Exception{
		...
		/*3.更新授信额度表*/
		HinsLimit hinsLimit=hinsLimitQuery.queryLimit(tempCredit.getUsername());
		if(hinsLimit==null){
			Map<String, Object> hinsLimitMap = getCreditHinsLimit(tempCredit);
			createCreditEvent.createHinsLimit(hinsLimitMap);
		}else{
			...
			createCreditEvent.updateHinsLimit(tempCredit.getUsername(),status);
		}
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> getCreditHinsLimit(HinsTempCredit tempCredit)
			throws Exception {
	...
	public Map<String, Object> getCreditMap(String appPayNo,HinsTempCredit tempCredit,CustCustomer custCustomer)
			throws Exception {
		...
		creditMap.put("hy_industry_code", tempCredit.getHy_industry_code());
		...
		creditMap.put("hins_pid", tempCredit.getHinsPid());
		return creditMap;
	}
