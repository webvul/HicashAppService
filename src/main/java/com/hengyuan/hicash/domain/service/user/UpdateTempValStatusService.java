 ...
 *  修改临时订单表
 */
public class UpdateTempValStatusService implements RespService  {

	
	private static Logger logger = Logger
			.getLogger(UpdateTempValStatusService.class);
	...
	public ParmResponse responseValue(ParmRequest parmRequest) {
		...
		try{
			...
			UpdateTempAppInfoResp upResp=query.queryTempSingleResp(req.getTempAppNo());
			 ...
			 if(upResp==null){
				 ...
				 if(valTypea.length()>9){
					 valTypea=valTypea.substring(8,10);//获取数据库节点
				...
				tempEvent.UpdateTempValStatus(req);
