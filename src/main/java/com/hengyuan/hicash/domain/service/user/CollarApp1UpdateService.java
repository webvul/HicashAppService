 ...
 *
 */
public class CollarApp1UpdateService implements RespService {
	
	private static Logger logger = Logger.getLogger(CollarApp1UpdateService.class);

	private CollarApp1UpdateEvent collarUpdateMsgEvent = new CollarApp1UpdateEvent();

	@Override
	public ParmResponse responseValue(ParmRequest parmRequest) {

		CollarApp1UpdateReq collarUpdateMsgReq = (CollarApp1UpdateReq) parmRequest;
		...
		try {
			...
			if(Consts.UPDATE_UNIVERSITY_TYPE_P.equals(collarUpdateMsgReq.getUpdateType())){
				...
				if(queryAppNoExist(collarUpdateMsgReq.getAppNo())){
					...
					if(!isSpouseMobile(collarUpdateMsgReq.getUserName(), collarUpdateMsgReq.getSpouseMobile())){
...
//						updateSpouseInfo(collarUpdateMsgReq);
						...
						resultCode = ResultCodes.STU_UPDATE_MOBILE_ILLEGAL;
					}
